...
function checkPushTime() {
  ...
  if ([9, 12, 14, 16, 18].includes(h) && lastPushKey !== key) {
    if(Notification.permission === "granted") new Notification("Check RA 93", { body: "Es gibt offene Aufgaben!" });
    lastPushKey = key;
  }
}
...
function initStreams() {
  ...
  onSnapshot(query(collection(db, "rides"), orderBy("createdAt", "desc"), limit(50)), s => {
    ...
    $("ridesList").innerHTML = s.docs.map(d => {
      ...
      return `<div class="item"><div class="main"><b>🚗 ${esc(r.name)}</b><br><small>Einsatz: ${esc(r.einsatz)}</small></div>${isAdmin ? `<button class="btn danger" onclick="delDoc('rides','${d.id}')">X</button>` : ''}</div>`;
    }).join("");
  });
}

// GLOBALE LÖSCHFUNKTION FÜR BUTTONS
...
function initAdminLogic() {
  onSnapshot(query(collection(db, "daily_tasks"), where("dateKey", "==", dayKeyNow()), where("status", "==", "done")), s => {
    ...
    s.docs.forEach(d => {
      const t = d.data();
      const html = `<div class="item"><div><b>${esc(t.text)}</b><br><small>${t.doneBy.join(", ")}</small></div><div class="row"><button class="btn danger" onclick="rejectTask('${d.id}')" style="margin-right:5px">❌</button><button class="btn ghost" onclick="finalCheck('${d.id}','${t.type}')">OK</button></div></div>`;
  ...
  onSnapshot(collection(db, "hygiene_templates"), s => {
    $("hygieneTemplateList").innerHTML = s.docs.map(d => `<div class="item"><span>${esc(d.data().text)}</span><button class="btn danger" onclick="delDoc('hygiene_templates','${d.id}')">X</button></div>`).join("");
  });
}

window.rejectTask = async (id) => {
  if(!confirm("Aufgabe ablehnen und wieder öffnen?")) return;
