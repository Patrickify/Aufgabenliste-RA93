...
function initAdminLogic() {
  onSnapshot(query(collection(db, "daily_tasks"), where("dateKey", "==", dayKeyNow()), where("status", "==", "done")), s => {
    ...
    s.docs.forEach(d => {
      const t = d.data();
      const html = `<div class="item"><span>${esc(t.text)} (${t.doneBy.join(",")})</span><div class="row"><button class="btn danger" onclick="window.rejectTask('${d.id}')">❌</button><button class="btn ghost" onclick="window.finalCheck('${d.id}','${t.type}')">OK</button></div></div>`;
...
window.rejectTask = async (id) => {
  if(!confirm("Aufgabe ablehnen?")) return;
...
function initPushSystem() {
  ...
  $("saveMuteBtn").onclick = async () => { const v = $("muteUntilInp").value; await updateDoc(doc(db, "users", auth.currentUser.uid), { muteUntil: v }); myMuteUntil = v; alert("OK"); };
  setInterval(() => {
    if (myMuteUntil && Number(myMuteUntil) >= Number(dayKeyNow())) return;
    const h = new Date().getHours();
    if ([9, 12, 14, 16, 18].includes(h) && new Date().getMinutes() === 0 && new Date().getSeconds() < 20) {
      if(Notification.permission === "granted") new Notification("RA 93: Aufgaben offen!");
    }
  }, 20000);
}

/* --- 7. GLOBALE ADMIN ACTIONS --- */
