...
function renderTagList() {
  ...
  if($("tagList")) $("tagList").innerHTML = tags.filter(t=>t.tagId.toLowerCase().includes(q)).map(t=>`
...
if($("markSelectedDoneBtn")) $("markSelectedDoneBtn").onclick = async () => {
  ...
  selectedTaskId = ""; if($("taskHint")) $("taskHint").textContent = "";
  document.querySelectorAll('input[name="worker"]').forEach(cb => cb.checked = false);
};

/* --- 4. ADMIN WOCHENPLAN & KONTROLLE --- */
function initAdminLogic() {
  ...
  onSnapshot(query(collection(db, "daily_tasks"), where("dateKey", "==", dayKeyNow()), where("status", "==", "done")), s => {
    if($("finalListTasks")) $("finalListTasks").innerHTML = s.docs.map(d => `<div class="item"><span>${esc(d.data().text)} (${d.data().doneBy.join(",")})</span><div class="row"><button class="btn danger" onclick="window.rejectTask('${d.id}')">❌</button><button class="btn ghost" onclick="window.finalCheck('${d.id}')">OK</button></div></div>`).join("");
  });
  
  onSnapshot(collection(db, "points_tasks"), st => onSnapshot(collection(db, "points_rides"), sr => renderPointsTable(st, sr)));
}

/* --- 5. AUTOMATIK, PUSH & TABS --- */
...
function initPushSystem() {
  ...
  if($("clearMuteBtn")) $("clearMuteBtn").onclick = async () => {
    if(auth.currentUser) { await setDoc(doc(db, "users", auth.currentUser.uid), { muteUntil: "" }, { merge: true }); alert("Urlaub beendet!"); }
  };
  setInterval(() => {
    if (myMuteUntil && Number(myMuteUntil) >= Number(dayKeyNow())) return;
    const h = new Date().getHours();
    if ([9, 12, 14, 16, 18].includes(h) && new Date().getMinutes() === 0 && Notification.permission === "granted") new Notification("Check RA 93!");
  }, 60000);
}
...
/* --- 6. ADMIN SETUP & ZUSATZAUFGABEN --- */
// Zusatzaufgabe für heute
...
// System & Mitarbeiter Setup
if($("empAddBtn")) $("empAddBtn").onclick = async () => {
  const v = n($("empAdd").value); if(!v) return;
  await setDoc(doc(db, "employees", keyOfName(v)), { name: v, passHash: "" });
  $("empAdd").value = "";
};
if($("adminUidAddBtn")) $("adminUidAddBtn").onclick = async () => {
  if(!isAdmin) return; await setDoc(doc(db, "admins_by_name", keyOfName($("adminUidAdd").value)), { enabled: true }); $("adminUidAdd").value = "";
};
if($("superUidAddBtn")) $("superUidAddBtn").onclick = async () => {
  if(!isSuperAdmin) return; await setDoc(doc(db, "superadmins_by_name", keyOfName($("superUidAdd").value)), { enabled: true }); $("superUidAdd").value = "";
};
if($("tagAddBtn")) $("tagAddBtn").onclick = async () => {
  const v = n($("tagAddInp").value); if(!v) return;
  await setDoc(doc(db, "tags", keyOfName(v)), { tagId: v, tagKey: keyOfName(v) }); $("tagAddInp").value = "";
};

// Wochenplan Vorlage speichern
...
// Hygiene Setup
if($("hygieneCatAddBtn")) $("hygieneCatAddBtn").onclick = async () => {
  const v = n($("hygieneCatInp").value); if(!v) return;
  await addDoc(collection(db, "hygiene_cats"), { title: v }); $("hygieneCatInp").value = "";
};
...
/* --- 7. HYGIENE CHECKLISTE & MODAL --- */
window.openHygCheck = async (id) => {
  activeCheckTaskId = id; const snap = await getDoc(doc(db, "daily_tasks", id)); const data = snap.data();
  if($("modalTitle")) $("modalTitle").textContent = data.text; 
  const cont = $("modalSubtasks"); if(cont) cont.innerHTML = "";
  if (!data.subtasks || data.subtasks.length === 0) { 
    if(confirm("Abschließen?")) { await updateDoc(doc(db, "daily_tasks", id), { status: "done", doneBy: [meName], doneAt: stamp() }); } return; 
  }
...
if($("regenTestBtn")) $("regenTestBtn").onclick = async () => { if(confirm("Reset?")) { const ex = await getDocs(query(collection(db, "daily_tasks"), where("dateKey", "==", dayKeyNow()))); const b = writeBatch(db); ex.forEach(d => b.delete(d.ref)); await b.commit(); location.reload(); } };
